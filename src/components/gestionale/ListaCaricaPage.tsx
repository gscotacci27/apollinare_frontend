...
import { ArrowLeft, Plus, Loader2, PackageX, Pencil, Trash2, X, Check } from 'lucide-react'
...
const QtaCell = ({ auto, manual }: { auto: number; manual: number }) => {
  ...
  return (
    <span className="tabular-nums">
      ...
      {manual > 0 && (
        <span className="text-indigo-400 text-xs ml-0.5">+{manual}</span>
      )}
    </span>
  )
}

// ── Riga editabile inline ──────────────────────────────────────────────────────
...
const EditRow = ({ item, idEvento, onCancel }: EditRowProps) => {
  ...
  return (
    <tr className="bg-indigo-950/30 border-b border-slate-800">
      <td className="px-4 py-3 text-sm text-slate-300 font-medium" colSpan={1}>
        {item.descrizione ?? item.cod_articolo}
        <p className="text-xs text-slate-500 mt-0.5">{item.cod_articolo}</p>
      </td>
...
const ItemRow = ({ item, idEvento, onEdit }: ItemRowProps) => {
  ...
  return (
    <tr className="border-b border-slate-800/60 hover:bg-slate-900/40 group transition-colors">
...
export const ListaCaricaPage = () => {
  ...
  const totalQta = lista.reduce(
    (sum, i) =>
      sum +
      (i.qta_ape + i.qta_man_ape) +
      (i.qta_sedu + i.qta_man_sedu) +
      (i.qta_bufdol + i.qta_man_bufdol),
    0,
  )
      ...
      <div className="px-6 py-4 border-b border-slate-800 flex items-center gap-3 shrink-0">
        ...
        {!isLoading && (
          <div className="text-right shrink-0">
            <p className="text-xs text-slate-400">{totalArticoli} articol{totalArticoli !== 1 ? 'i' : 'o'}</p>
            <p className="text-xs text-slate-600">{totalQta} pezzi tot.</p>
          </div>
        )}
        <button
          ...
          <table className="w-full text-sm">
            <thead className="sticky top-0 bg-slate-900 border-b border-slate-800 z-10">
              <tr>
                ...
                <th className="text-center px-3 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider w-20">
                  Ape
                </th>
                <th className="text-center px-3 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider w-20">
                  Sedu
